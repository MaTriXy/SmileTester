            ...
            for result in results {
                if let landmarks = result.landmarks, let points = landmarks.allPoints {
                    ...
                    DispatchQueue.main.async {
                        ...
                        //let value = formatter.string(from: NSNumber(value: (smileOutput.output1Probability[smileOutput.output1] ?? 0) * 100))
                        let value = smileOutput.output1[0].doubleValue
                        
                        let percent = "\(value*100)%"
                        
                        let isSmiling = value > 0.5
                        if isSmiling {
